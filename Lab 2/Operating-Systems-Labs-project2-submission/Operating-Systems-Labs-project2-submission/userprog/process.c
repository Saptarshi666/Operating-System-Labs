...
tid_t process_execute(const char* file_name) {
  ...
  if (youngin != NULL) {
    ...
    strlcpy(youngin->directory, current->directory,
            strlen(current->directory) + 1);
  }
  // printf("%s\n", current->directory);
  return tid;
}
...
void process_exit(void) {
  // printf("\t### DEBUG: process.exit()\n");
  struct thread* curThread = thread_current();

  check_lock();

  remove_File();
  if (curThread->file != NULL) {
    file_allow_write(curThread->file);
  }
  ...
  for (; parser != list_end(&curThread->inheritors);
       parser = list_next(parser)) {
    youngin = list_entry(parser, struct thread, inheritorelem);
    sema_up(&youngin->stop);
    sema_down(&youngin->wait);
  }
...
bool load(const char* file_name, void (**eip)(void), void** esp) {
  log(L_TRACE, "load()");
  // printf("\t### DEBUG: process.load(file_name='%s')\n", file_name);
  struct thread* t = thread_current();
  ...
  if (kpage != NULL) {
    ...
    if (success) {
      *esp = PHYS_BASE;
      ...
      const char*
          arguments_stack[80];  // Hold addresses of arguments in the stack
      ...
      /*Write each argument (including the executable name) in reverse order, as
      well as in reverse for each string, to the stack.*/
      int copy_i = i - 1;
      while (copy_i != -1) {
        *esp -= 1;                          // For null character
        *esp -= strlen(arguments[copy_i]);  // Save memory space for argument
        arguments_stack[copy_i] =
            memcpy(*esp, arguments[copy_i],
                   strlen(arguments[copy_i]));  // Write argument to stack
        copy_i--;
      }
      ...
      *esp -= sizeof(void*);
      memset(*esp, 0x0, sizeof(void*));

      return success;
    } else {
      palloc_free_page(kpage);
    }
    // hex_dump( *(int*)esp, *esp, 128, true ); // NOTE: uncomment this to check
    // arg passing
  }
  return success;
}
