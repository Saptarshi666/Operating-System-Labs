...
/*has the functionality of string token and gives us the file to be run*/
...
static void syscall_handler(struct intr_frame* f UNUSED) {
  ...
  switch (sysnumber) {
    ...
    case SYS_CREATE: /* Create a file. */
      *&f->eax =
          (uint32_t)create(*(((char**)esp) + 1), *(((unsigned*)esp) + 2));
      break;
    ...
    case SYS_READ: /* Read from a file. */
      *&f->eax = (uint32_t)read(*(((int*)esp) + 1), (void*)*(((int**)esp) + 2),
                                *(((unsigned*)esp) + 3));
      break;

    case SYS_WRITE: /* Write to a file. */
      *&f->eax = (uint32_t)write(*(((int*)esp) + 1), (void*)*(((int**)esp) + 2),
                                 *(((unsigned*)esp) + 3));
      break;
    ...
    default:
      //  printf("\t### DEBUG: UNHANDLED SWITCH CASE: %d", sysnumber);
      break;
  }
}
...
// returns filesize
int filesize(int fd) {
  //  printf("\t### DEBUG: syscall.filesize(fd=%d)\n", fd);
  ...
  int size = file_length(fdesc->file); /*Thread blocking the file so no other
...
unsigned tell(int fd) {
  // printf("\t### DEBUG: syscall.tell(fd=%d)\n", fd);
  struct file_desc* fdesc = findFile(thread_current(), fd);
  ...
  lock_acquire(&filesys_lock);
  unsigned result = file_tell(fdesc->file); /*Thread blocking the file so no
                                               other one tries to access it*/
  lock_release(&filesys_lock);

  return result;
}
/*turn it into a directory path for file to be accessed*/
void inversion(const char* relative, char* buffer) {
  // printf("\t\t### DEBUG: syscall.inversion(relativePath='%s')\n", relative);
  if (*relative == '/') {
  ...
  strlcpy(cur_copy, thread_current()->directory,
          strlen(thread_current()->directory) + 1);
  ...
  if (j == 0) {
    ...
    for (k = 0; k < j; k++) {
      ...
      strlcat(buffer, cur_splitStrings[k],
              strlen(buffer) + strlen(cur_splitStrings[k]) + 1);
    }
  }
...
int open(const char* file) {
  // printf("\t### DEBUG: syscall.open(file='%s')\n", file);
  if (strcmp(file, "") == 0)
    return -1;
  ...
  if (result) {
    ...
    lock_acquire(&filesys_lock);
    new->file = filesys_open(splitStrings[i]);
    lock_release(&filesys_lock);
...
bool remove(const char* file) {
  // printf("\t### DEBUG: syscall.remove(file='file')\n", file);
  if (!user_check(file)) {
    exit(-1);
  }
  ...
  if (result) {
    lock_acquire(&filesys_lock);
    result = filesys_remove(splitStrings[i]);
    lock_release(&filesys_lock);
  }
...
bool create(const char* file, unsigned initial_size) {
  //  printf("\t### DEBUG: syscall.create(file='%s', size=%u)\n", file,
  //     initial_size);
  if (!user_check(file)) {
    exit(-1);
  }
  ...
  if (result) {
    lock_acquire(&filesys_lock);
    result = filesys_create(splitStrings[i], initial_size);
    lock_release(&filesys_lock);
  }
...
int read(int fd, void* buffer, unsigned size) {
  // printf("\t### DEBUG: syscall.read(fd=%d, size=%u)\n", fd, size);
  int bytecount = 0;
  ...
  if (fd == 0) {
    ...
    lock_acquire(&filesys_lock);
    bytecount =
        file_read(fdesc->file, buffer, size); /*Thread blocking the file so no
                                                 other one tries to access it*/
    lock_release(&filesys_lock);
  }
...
void close(int fd) {
  // printf("\t### DEBUG: syscall.close(fd=%d)\n", fd);
  struct file_desc* fdesc = findFile(thread_current(), fd);
  ...
  lock_release(&filesys_lock); /*Thread blocking the file so no other one
                                  tries to access it*/
  // }
...
void seek(int fd, unsigned position) {
  // printf("\t### DEBUG: syscall.seek(fd=%d, position=%u)\n", fd);
  struct file_desc* fdesc = findFile(thread_current(), fd);

  if (fdesc) {
    lock_acquire(&filesys_lock);
    file_seek(fdesc->file, position); /*Thread blocking the file so no
                                         other one tries to access it*/
    lock_release(&filesys_lock);
  }
...
int write(int fd, const void* buffer, unsigned size) {
  if (fd >= 3) {
    // printf("\t### DEBUG: syscall.write(fd=%d, size=%u)\n", fd,
    //  size);
  }
  int bytecount = 0;
  ...
  if (fd == 1) {
    ...
    lock_acquire(&filesys_lock);
    bytecount =
        file_write(fdesc->file, buffer, size); /*Thread blocking the file so no
                                                  other one tries to access it*/
    lock_release(&filesys_lock);
  }
...
static bool user_check(
    const char* uaddr) {  // checks to see if user memory is valid
