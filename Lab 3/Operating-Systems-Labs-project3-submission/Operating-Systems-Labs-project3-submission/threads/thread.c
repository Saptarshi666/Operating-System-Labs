...
/* Compare the sleep time (ticks) of each input thread */
...
/* Compare the priority of input threads */
bool cmp_priority(const struct list_elem* ele1,
                  const struct list_elem* ele2,
                  void* aux UNUSED) {
...
/* Let current running thread sleep for TIMER ticks by pushing current thread to
 * sleeping list, set status as Thread sleeping and wake time as current time
 * plus ticks */
...
/* Compare the current thread's priority with the thread that has the highest
 * priority in ready list, and yield to it. */
...
/* Starts preemptive thread scheduling by enabling interrupts. Also creates the
 * idle thread. */
...
tid_t thread_create(const char* name,
  ...
  /* For userprog, adding child process */
...
/* Sets the current thread's priority to NEW_PRIORITY. To pass priority change
 * test, verify that lowering a thread's priority so that it is no longer the
 * highest-priority thread in the system causes it to yield immediately. */
...
static void init_thread(struct thread* t, const char* name, int priority) {
  ...
  t->file_desc = 2;  // 0=STDIN, 1=STDOUT, 2=STDERR
  t->load_status = false;
...
static void update_sleep_thread_schedule(void) {
...
/* Case 1: the simple situation is there are two threads, one lock holder
   ...
   Case 2: for the nest lock situation, iteratively update previous lock
   holder's priority until its waiting lock is null.
 */
...
/* To handle multiple donation, we need to remvove the thread which are waiting
 * for this lock when realeasing this lock. */
...
void update_priority(void) {
  ...
  struct thread* front_thread = list_entry(
      list_front(&current_thread->donor_list), struct thread, donor_elem);
...
struct child_thread_data* thread_get_child_data(struct thread* parent,
                                                tid_t child_tid) {
#ifdef USERPROG
  struct list_elem* it;
  for (it = list_begin(&parent->children_data);
       it != list_end(&parent->children_data); it = list_next(it)) {
    struct child_thread_data* t =
        list_entry(it, struct child_thread_data, elem);
    if (t->tid == child_tid) {
      return t;
    }
  }
