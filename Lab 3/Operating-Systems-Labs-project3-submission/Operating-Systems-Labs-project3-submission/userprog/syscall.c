...
static inline bool get_user1(uint8_t* dst, const uint8_t* usrc);
...
/* Copies SIZE bytes from user address USRS to kernel address DST. Call
...
static void copy_in(void* dst_, const void* usrc_, size_t size) {
  ...
  for (; size > 0; size--, dst++, usrc++)
    if (usrc >= (uint8_t*)PHYS_BASE || !get_user1(dst, usrc))
      exit(-1);
}

/* Creates a copy of user string US in kernel memory and returns it as a page
 * that must be freed with palloc_free_page(). Truncates the string at PGSIZE
 * bytes in size. Call thread_exit() if any of the user accesses are invalid. */
static char* copy_in_string(const char* us) {
  char* ks;
  size_t length;
  ...
  for (length = 0; length < PGSIZE; length++) {
    if (us >= (char*)PHYS_BASE || !get_user1(ks + length, us++)) {
      palloc_free_page(ks);
      thread_exit();
    }
 ...
 * below PHYS_BASE. Returns true if successful, false if a segfault occurred. */
static inline bool get_user1(uint8_t* dst, const uint8_t* usrc) {
  int eax;
...
/* Finds file handle for given file discriptor by searching in list of files
 * owned */
struct file* get_file_handle(int file_desc) {
...
void exit(int status) {
  ...
  thread_get_child_data(current->parent, current->tid)->exit_status =
      current->exit_status;
  thread_exit();
}

/* syscall for read, returns no of bytes read from buffer or file STDIN_FILENO
 * is reading from buffer */
...
/* syscall for create, calls file_create */
bool create(const char* file, unsigned initial_size) {
  bool success = false;
  if (!file || !(file < PHYS_BASE &&
...
/* File size sys call, gets file handle and then returns, file_length of file */
int filesize(int fd) {
  int size;
...
/* syscall for tell, gets file handle and calls file_tell */
unsigned tell(int fd) {
  unsigned pos;
...
/* syscall for close , gets file handle and closes file */
void close(int fd) {
...
/* syscall for exec, calls process_execute */
int exec(const char* cmd_line) {
  if (!cmd_line)
    return -1;
...
void close_all_files(void) {
  ...
  while (el != list_end(&thread_current()->files_owned_list)) {
    struct file_struct* fs = list_entry(el, struct file_struct, elem);
    nxt = list_next(el);
    file_close(fs->file);
    list_remove(&fs->elem);
    free(fs);
