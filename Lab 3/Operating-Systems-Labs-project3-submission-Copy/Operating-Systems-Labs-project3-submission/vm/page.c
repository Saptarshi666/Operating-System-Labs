...
struct struct_page* vm_add_new_page(void* address,
                                    ...
                                    bool writable,
                                    off_t block_id) {
  struct struct_page* new_page =
      (struct struct_page*)malloc(sizeof(struct struct_page));

  if (new_page == NULL) {
    return NULL;
  }

  new_page->type = 1;  // type=file
  ...
  add_page_to_pagedir(new_page->pointer_to_pagedir, new_page->address,
                      (void*)new_page);

  return new_page;
}

struct struct_page* vm_add_new_zeroed_page(void* addr, bool writable) {
  struct struct_page* zero_page =
      (struct struct_page*)malloc(sizeof(struct struct_page));
  if (zero_page == NULL) {
    return NULL;
  }
  ...
  add_page_to_pagedir(zero_page->pointer_to_pagedir, zero_page->address,
                      (void*)zero_page);

  return zero_page;
}

bool vm_load_new_page(struct struct_page* new_page, bool is_pinned) {
  ...
  if (new_page->type == 1) {
    sys_filelock(true);  // lock to sync this block
    file_seek(new_page->file.file_name, new_page->file.ofs);
    size_t bytes_read =
        file_read(new_page->file.file_name, new_page->frame_page,
                  new_page->file.read_bytes);
    sys_filelock(false);
    ...
    if (flag) {
      memset(new_page->frame_page + new_page->file.read_bytes, 0,
             new_page->file.zero_bytes);
    }
  ...
  if (!pagedir_set_page(new_page->pointer_to_pagedir, new_page->address,
                        new_page->frame_page, new_page->is_writable)) {
    ASSERT(false);
    unpin(new_page->frame_page);
    return false;
  }
...
void vm_unload(struct struct_page* p, void* fpage) {
  lock_acquire(&page_unload_lock);
  if (&p->type == 1 && pagedir_is_dirty(p->pointer_to_pagedir, p->address) &&
      file_writable(p->file.file_name) == false) {
    pin(fpage);
    sys_filelock(true);  // lock for sync
    file_seek(p->file.file_name, p->file.ofs);
    file_write(p->file.file_name, fpage, p->file.read_bytes);
    sys_filelock(false);
    unpin(fpage);
  } else if (&p->type == 2 ||
             pagedir_is_dirty(p->pointer_to_pagedir, p->frame_page)) {
    p->type = 2;
...
struct struct_page* vm_find_page_in_supplemental_table(void* addr) {
...
/* check whether address is in user space and in range of 32 bytes from stack
 * pointer */
bool is_stack_access_vaid(const void* esp, void* addr) {
  return (uint32_t)addr > 0 && addr >= (esp - 32) &&
         (PHYS_BASE - pg_round_down(addr)) <= (1 << 23);
}

struct struct_page* vm_add_zeroed_page_on_stack(void* vaddr, bool pinned) {
...
void vm_free_this_page(struct struct_page* page) {
  if (page == NULL) {
    return;
  }
