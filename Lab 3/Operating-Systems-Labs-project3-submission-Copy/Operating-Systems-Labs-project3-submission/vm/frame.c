...
int num = 1;
static struct list list_of_frames;  // frames list
...
/* Initializes frames hash, called from threads/init.c from main function */
...
struct struct_page* get_page_from_frame(void* fr, uint32_t* page_dir) {
  ...
  for (e = list_begin(&f->frame_pages); e != list_end(&f->frame_pages);
       e = list_next(e)) {
...
unsigned frame_hash(const struct hash_elem* e_, void* aux UNUSED) {
  const struct struct_frame* sf =
      hash_entry(e_, struct struct_frame, hash_elem);
...
bool frame_comparator(const struct hash_elem* first_,
                      ...
                      void* aux UNUSED) {
  const struct struct_frame* f =
      hash_entry(first_, struct struct_frame, hash_elem);
  const struct struct_frame* s =
      hash_entry(second_, struct struct_frame, hash_elem);
...
/* find frame based on a given page */
static struct struct_frame* find_frame(void* pg) {
...
void* get_frame(enum palloc_flags flags) {
  ...
  if (pg != NULL) {
    ...
    /* pinn the frame so it can not evict before caller loads data on it */
    vf->pin = 1;
    vf->page = pg;
...
static struct struct_frame* get_next_page_for_eviction(void) {
  ...
  if (next_to_evict != NULL) {
    struct struct_frame* f =
        list_entry(next_to_evict, struct struct_frame, page_elem);
    return f;
  }
...
static void circular_evict(void) {
  ...
  while (frame_to_evict == NULL) {
    ...
    frame_to_evict = f;
    //   printf("frame being evicted is %d \n", f->num);
  }
