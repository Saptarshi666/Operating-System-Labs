...
bool add_page_to_pagedir(uint32_t* page_dir,
                         void* user_page,
                         void* struct_page) {
...
void* find_page_in_pagedir(uint32_t* pagedir, const void* uaddr) {
  uint32_t* page_table_entry;
  ...
  if (page_table_entry != NULL) {
    if ((*page_table_entry & PTE_P) != 0) {
      void* page = pte_get_page(*page_table_entry) + pg_ofs(uaddr);
